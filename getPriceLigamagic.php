...
<?php getPrecoBanco(); ?>
    ...
    const myChart = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          data: [
            // pegar depois os valores com javasctipt do server
            <?php 
              $sqlDashboard = "SELECT * FROM `colecoes`";
              $rsDashboard = mysqli_query($GLOBALS["conn"],$sqlDashboard);
              foreach($rsDashboard as $arrDash) {
                  echo "$arrDash[qtcomum]"; echo ",";
                  echo "$arrDash[qtincomum]"; echo ",";
                  echo "$arrDash[qtrara]"; echo ",";
                  echo "$arrDash[qtmitica]";
              }
            ?>
          ],
          lineTension: 0,
